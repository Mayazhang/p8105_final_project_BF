...
crime = crime_raw %>% 
  #readxl::read_excel("../data/crime-data-2016.xls", range = ("A4:W202")) %>% 
  janitor::clean_names() %>% 
...
#state_transform <- state.abb
#names(state_transform) <- state.name

#crime_clean_rate <- crime_clean_rate %>% mutate(state = state_transform[state])
#crime_clean_rate <- crime_clean_rate %>% mutate(state = as.character(state))
```
