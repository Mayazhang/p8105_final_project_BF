...
drug_cocaine = 
  ...
  group_by(year) %>%
  summarize(mean_past_year_cocaine = mean(illicit_drugs_cocaine_used_past_year))
 
fit_cocaine = fitted(loess(drug_cocaine$mean_past_year_cocaine ~ drug_cocaine$year))
 
drug_cocaine %>%
  plot_ly(x = ~year, y = ~mean_past_year_cocaine, type = "bar", showlegend = FALSE, marker = list(color = ~year, showscale = FALSE)) %>% 
  add_lines(y = fit_cocaine, showlegend = FALSE, color = 'black')
```

```{r drug_cocaine_state}
drug_clean %>% 
  select(year, state, age, contains("cocaine")) %>%
  group_by(state) %>%
  summarize(
