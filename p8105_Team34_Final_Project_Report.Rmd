...
library(tidyverse)
>>>>>>> 3fd265d2023410a28c732f40d9d68d60d55f58db
```
...
drug_clean %>% 
  ...
  select(-other) %>% 
  group_by(year, drug) %>% 
  summarize(average_rate = sum(average_rate)) %>% 
  ggplot(aes(x = as.factor(year), y = average_rate)) +
  geom_bar(aes(fill = drug), stat = "identity", width = 0.6) +
  theme_bw() +
...
=======
After we fit the models and obtain the results for each regression, we filter the estimated coefficients with p-value less than 0.05, we found that there are two outcomes (murder/manslaughter, robbery) are significantly related to drug use (illicit drug and marijuana). Then we run the diagnostics for these two multivariable linear regression models. They are all good with model assumptions.
>>>>>>> 3fd265d2023410a28c732f40d9d68d60d55f58db

In conlusion, at 0.05 significant level, murder/manslaughter is negatively associated with illicit drug, while robbery and murder/manslaughter are positively associated with marijuana use.
