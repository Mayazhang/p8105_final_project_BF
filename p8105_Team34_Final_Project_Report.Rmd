...
After we fit the models and obtain the results for each regression, we filter the estimated coefficients with p-value less than 0.05, we found that there are two outcomes are significantly related to drug use. Then we run the diagnostics for these two multivariable linear regression models. They are all good with model assumptions.

In conlusion, at 0.05 significant level, murder/manslaughter is negatively associated with illicit drug, while burglary and murder/manslaughter are positively associated with marijuana use.
