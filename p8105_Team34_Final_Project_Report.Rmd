...
We try to remove these two outliers, and then fit models again. 
`r datatable(result_withoutoutliers, options = list(pageLength = 10))`

The R-squared and adjusted R-squared increase by approximately 0.3, indicating a higher goodness of fit. However, the estimated coefficients of drug use become unsiginficent (p-value > 0.05).
