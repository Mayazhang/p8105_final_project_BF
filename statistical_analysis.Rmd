...
crime_clean = 
  crime %>%
  filter(year != "NA") %>% 
  rename(state = area) #Change the name of "area" to "state"
```
...
race_data_raw = gsheet2tbl('https://docs.google.com/spreadsheets/d/1Xw2v4feQPLEVql-aTEyYCv4cQ_dOFB1qclt-6dGlGxU/edit?usp=sharing') %>% janitor::clean_names() %>%
  select(state, white)
