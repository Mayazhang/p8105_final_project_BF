...
# up to here, it's already a tidy dataset, but for regression we need to define each degree in each drug as a variable, so we choose to spread "rate" column
  spread(key = rate, value = drug_sum)
...
model1 = lm(burglary_rate ~ illicit_sum + alcohol_sum + marijuana_sum, data = regression_data_raw)
par(mfrow = c(2,2))
plot(model1)
```
