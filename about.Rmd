...
```{r drug dataset load and clean up, echo=FALSE}
...
```{r prepare the outcome and predictors, echo=FALSE}
# clean drug data
...
```{r read in new dataset, echo=FALSE}
...
```{r distribution checking, echo=FALSE}
# Check distributions 
regression_data_all %>%
  select(-state) %>%
  chart.Correlation()
...
```{r linear regression_another, echo=FALSE}
# MLR linear regression using list columns
...
```{r model diagnostics_another, echo=FALSE}
# Fit models and then check the assumption
